...
singularity exec --nv  /home/groups/akundaje/anusri/simg/chrombpnet_latest/chrombpnet_bigwig_new.sif chrombpnet bias pipeline -ibw $oak_dir/preprocessing/bigWigs/$experiment".bigWig" -d $data_type -g $reference_dir/hg38.genome.fa -c  $reference_dir/chrom.sizes -p $oak_dir/preprocessing/downloads/peaks.bed.gz -n $oak_dir/preprocessing/negatives_data_new/fold_"$fold"/"$experiment"_negatives.bed -fl /scratch/groups/akundaje/anusri/chromatin_atlas/splits/fold_$fold".json" -b $bias_threshold -o $output_dir -fp $experiment
