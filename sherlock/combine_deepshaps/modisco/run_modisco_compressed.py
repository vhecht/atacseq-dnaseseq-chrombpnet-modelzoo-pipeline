...
if __name__=="__main__":
    ...
    scores = np.transpose(scores['shap']['seq'],(0,2,1))

    regions_df = pd.read_csv(args.regions, sep='\t', names=NARROWPEAK_SCHEMA)
    genome = pyfaidx.Fasta(args.genome)

    seqs = context.get_seq(regions_df, genome, scores.shape[1])
    raw = np.transpose(seqs, (0, 2, 1))
    projected_shap = np.transpose(seqs*scores, (0, 2, 1))

    for i in projected_shap:
