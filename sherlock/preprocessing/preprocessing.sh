...
then
    ...
    do
        ...
        echo $( timestamp ): "samtools view -@4 -F 780 -q 30 -b" $bam_file_path \
        "-o" $intermediates_dir/${bam_file}.bam | tee -a $logfile
        samtools view -@4 -F 780 -q 30 -b $bam_file_path \
...
then
    ...
    echo $( timestamp ): "samtools merge -@4 -f" \
    $intermediates_dir/$experiment$tag.bam ${all_bams_for_merging[*]} | \
    tee -a $logfile
    samtools merge -@4 -f $intermediates_dir/$experiment$tag.bam \
    ${all_bams_for_merging[*]}
...
echo $( timestamp ): "samtools sort  -@4  $intermediates_dir/$experiment$tag.bam -o $intermediates_dir/sorted_$experiment$tag.bam"
samtools sort -@4 $intermediates_dir/$experiment$tag.bam -o $intermediates_dir/sorted_$experiment$tag.bam
...
if [ "$assay_type" = "DNase-seq" ] ; then
    ...
    echo $( timestamp ): "samtools view -@4 -b" $intermediates_dir/sorted_$experiment$tag.bam "|" "bedtools bamtobed -i stdin" "|" \
    ...
    samtools view -@4 -b $intermediates_dir/sorted_$experiment$tag.bam | bedtools bamtobed -i stdin | \
    awk -v OFS="\t" '{if ($6=="-"){print $1,$2,$3+1,$4,$5,$6} else if ($6=="+") {print $1,$2,$3,$4,$5,$6}}' | \
    bedtools genomecov -bg -5 -i stdin -g $reference_dir/chrom.sizes | bedtools sort -i stdin > $intermediates_dir/$experiment$tag.bedGraph
